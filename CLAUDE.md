...
- `config.ts` — `~/.resident/config.json`: repos (path/name/checks/pull), urls, budgets,
  intervalMinutes, model, bind, notify, noise rhythm
- `senses.ts` — polling watchers: git/deps/typecheck/uptime/github/sentry → scored `Finding`s
- `store.ts` — SQLite findings + per-repo memory notebook + earned authority + delivery queue; statuses: queued→investigating→ready→(approving→approved→
  merged|closed)|(working→tracked)|failed|ignored|dismissed; INVESTIGATE_THRESHOLD = 55
- `daemon.ts` — cycle loop: outcomes/auto-merge → noise delivery → blindness → senses → triage → budgeted
...
- `server.ts` — inbox API: /api/state, /api/cycle, /api/watch, /api/memory, /api/noise, /api/autonomy, /api/item/:id/{approve,issue,
