...
Config lives in `~/.resident/config.json`: repos, watched URLs, cycle interval, budgets, and optional `"notify"` — point it at an [ntfy](https://ntfy.sh) topic URL or a Slack incoming webhook and Resident pings your phone when something's ready for you, when a PR opens, or when a site goes down. Add `"quietHours": { "start": "22:00", "end": "08:00" }` to silence calm pings overnight (work continues, the inbox never sleeps) — site-down and blindness alarms, plus feedback for actions you just clicked, still pierce the window — and `"digest": "08:30"` to get one morning summary — what's ready, what was spent — instead of a night of pings. State in `~/.resident/resident.db`, investigation transcripts in `~/.resident/runs/` (set `RESIDENT_HOME` to relocate state for foreground runs and tests — the launchd install always uses `~/.resident`). The watchlist is also editable from the inbox itself (the chips row).
