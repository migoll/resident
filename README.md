...
- **Judges** every finding with a score. Below threshold → logged visibly with its reasoning (you can always audit what it chose to ignore). Above → queued for investigation, capped by budgets (default 2/cycle, 10/day). Your dismissals teach it: the third "no" on a kind-in-a-repo auto-mutes that kind — a 🔇 chip in the inbox (showing what it's currently holding back), one tap to reverse. Outage and fatal-error kinds are exempt from learning — those only go quiet if you mute them deliberately — and blindness alerts can never be muted at all.
...
Config lives in `~/.resident/config.json`: repos, watched URLs, cycle interval, budgets, and optional `"notify"` — point it at an [ntfy](https://ntfy.sh) topic URL or a Slack incoming webhook. Quiet hours and an optional morning digest keep ordinary pings calm; the inbox also exposes a reversible Silent setting and an optional Monday outcome report. Blindness and site-down alarms remain loud. Per-repo/kind authority is earned only from real accepted work: a human approves each upgrade (shadow → bounded low-risk auto-PR → CI-green auto-merge) and can revoke it in the inbox. Related investigations share a canonical root-cause card rather than competing for attention. State in `~/.resident/resident.db`, investigation transcripts in `~/.resident/runs/` (set `RESIDENT_HOME` to relocate state for foreground runs and tests — the launchd install always uses `~/.resident`).
...
Everything autonomous is **read-only by design** until you approve an earned authority upgrade. Default shadow mode requires a human click, and every write happens on a disposable worktree branch, never your checkout, never `main`. Auto-PR authority is limited to model-rated low-risk diffs of at most 40 changed lines across two files; auto-merge needs a second grant, green checks, and GitHub-confirmed mergeability. Budgets cap the AI spend per cycle and per day.
