...
import { deliver, flushNoise, weeklySummary } from './noise'
import { INVESTIGATE_THRESHOLD, type Item, type Store, type Trust } from './store'
...
export function autoPrEligible(item: Pick<Item, 'patch' | 'command' | 'evidence'>): boolean {
  if (!item.patch || item.command) return false // command fixes remain a deliberate per-click capability
...
function checksGreen(checks: any): boolean {
  return Array.isArray(checks) && checks.length > 0 && checks.every((check) => ['SUCCESS', 'NEUTRAL', 'SKIPPED'].includes(check?.conclusion))
}
...
async function maybeAutoMerge(cfg: Config, store: Store, item: Item, log: (s: string) => void) {
  ...
  try {
    ...
    const merged = await run(['gh', 'pr', 'merge', item.pr_url!, '--squash', '--delete-branch'], undefined, 120_000)
    if (!merged.ok) return false
    const after = await run(['gh', 'pr', 'view', item.pr_url!, '--json', 'state'])
    if (after.ok && JSON.parse(after.stdout).state === 'MERGED') {
      store.update(item.id, { status: 'merged', reason: 'auto-merged under earned authority (small low-risk diff, CI green)' })
      await deliver(cfg, store, 'Resident: auto-merged', `${item.title}\n${item.pr_url}`)
      log(`  ✓ auto-merged: ${item.title}`)
      return true
    }
  } catch {}
  return false
...
/** Authority is earned from real human decisions, never switched on silently. */
...
function proposeAutonomy(store: Store) {
  for (const trust of store.trust()) {
    const mode = autonomyProposal(store, trust)
    if (!mode) continue
    const label = mode === 'auto_pr' ? 'auto-PR' : 'auto-merge'
    store.upsertFinding({
      hash: `autonomy|${trust.repo}|${trust.kind}|${mode}`,
      sense: 'autonomy', repo: trust.repo, kind: `grant-${mode.replace('_', '-')}`,
      target_kind: trust.kind,
      title: `Grant ${label} authority for ${trust.kind} findings in ${trust.repo}?`,
      detail: `${trust.accepted}/${trust.total} human decisions accepted (${Math.round(trust.rate * 100)}%); ${trust.merged} merged PR${trust.merged === 1 ? '' : 's'}.\n\n${mode === 'auto_pr'
        ? 'Resident will only open a PR for small (≤40 changed lines, ≤2 files), low-risk diffs. Commands still require a click.'
        : 'Resident may additionally squash-merge only those small low-risk auto-PRs after every required check is green and GitHub says the PR is mergeable.'}\n\nYou can revoke this authority at any time in the inbox.`,
      score: 60, status: 'ready', target_kind: trust.kind,
    })
  }
}
...
async function maybeAutoApprove(cfg: Config, store: Store, item: Item, repoPath: string, log: (s: string) => void) {
  ...
  log(`  ⚒ auto-PR: ${item.title} [${item.repo}]`)
  const res = await approve(item, repoPath, applyModel(cfg))
  store.addCost(res.cost)
  if (res.ok) {
    store.update(item.id, { status: 'approved', pr_url: res.pr_url, cost: item.cost + res.cost, reason: 'auto-PR under earned authority (small low-risk diff)' })
    await deliver(cfg, store, 'Resident: auto-PR opened', `${item.title}\n${res.pr_url}`)
    log(`    → ${res.pr_url}`)
  } else {
    store.update(item.id, { status: 'failed', cost: item.cost + res.cost, reason: 'auto-PR failed — see runs log' })
    await deliver(cfg, store, 'Resident: auto-PR failed', item.title)
    log('    → auto-PR failed')
  }
  return true
}
...
export async function cycle(
  ...
  await flushNoise(cfg, store, log)
  await weeklySummary(cfg, store, log)
  ...
  if (budget > 0) {
    for (const item of store.queued(budget)) {
      ...
      if (!repo) {
        ...
        await deliver(cfg, store, 'Resident: alert', item.title, 'critical')
        continue
      }
      ...
      if (res.ok) {
        ...
        if (duplicateOf) {
          log(`    ↳ collapsed into finding #${duplicateOf} (same root cause)`)
        } else if (!(await maybeAutoApprove(cfg, store, ready, repo.path, log))) {
          log(`    → ready (${fix}, ${model}, $${res.cost.toFixed(2)})`)
          await deliver(cfg, store, 'Resident: ready for you', `${item.title} [${item.repo}]${res.patch ? ' — fix proposed, one tap to PR' : res.command ? ` — command proposed: ${res.command}` : ''}`)
        }
      } else {
